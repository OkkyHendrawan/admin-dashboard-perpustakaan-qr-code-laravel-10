...
class AnggotaController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['header_tittle']= "Daftar Anggota";
        return view('admin.anggota.list', compact('anggota'), $data);
    }

    // Menampilkan form untuk menambahkan anggota baru
    public function form_create()
    {
        $data['header_tittle']= "Tambah Anggota";
    ...
    public function proses_create(Request $request)
    {
        ...
        $request->validate([
            ...
            'anggota_nama' => 'required|string|max:255|unique:anggota,anggota_nama',
            'anggota_jenis' => 'nullable|in:Aktif,Tidak Aktif',
            'anggota_tgl_daftar' => 'nullable|date',
        ]);

        // Ambil kode anggota terakhir dari database
        $lastAnggota = AnggotaModel::orderBy('anggota_kode', 'desc')->first();

        // Buat kode anggota baru
        if ($lastAnggota) {
            $lastKode = (int)substr($lastAnggota->anggota_kode, 1);
            $newKode = 'A' . ($lastKode + 1);
        } else {
            $newKode = 'A1'; // Jika belum ada anggota, mulai dari A1
        }

        // Simpan data anggota baru ke dalam database
        $anggota = new AnggotaModel;
        $anggota->anggota_kode = $newKode;
    ...
    public function edit($anggota_id)
    {
        ...
        $data['header_tittle']= "Edit Anggota";
        return view('admin.anggota.edit', compact('anggota'), $data);
    }
    ...
    public function softDeleteAnggota($anggota_id)
        ...
        {
            $data['header_tittle']= "Mencari Anggota";
