        ...
        <div
            ...
            <div class="container-fluid group-data-[content=boxed]:max-w-boxed mx-auto">
                <div class="flex flex-col gap-2 py-4 md:flex-row md:items-center print:hidden">
                    <div class="grow">
                        <h5 class="text-16">Tambah Peminjaman Baru</h5>
                    </div>
                    <ul class="flex items-center gap-2 text-sm font-normal shrink-0">
                        <li
                            class="relative before:content-['\ea54'] before:font-remix ltr:before:-right-1 rtl:before:-left-1 before:absolute before:text-[18px] before:-top-[3px] ltr:pr-4 rtl:pl-4 before:text-slate-400 dark:text-zink-200">
                            <a href="{{ route('admin.peminjaman.list') }}" class="text-slate-400 dark:text-zink-200">Daftar
                                Peminjaman</a>
                        </li>
                    ...
                    <div class="block tab-pane" id="peminjamanTabs">
                        <div class="card">
                            <div class="card-body">
                                ...
                                <p class="mb-4 text-slate-500 dark:text-zink-200">Isi informasi peminjaman baru di sini.</p>
                                <form method="POST" action="{{ route('admin.peminjaman.proses_create') }}">
                                    @csrf
                                    <div class="grid grid-cols-1 gap-5 xl:grid-cols-12">
                                        <div class="xl:col-span-6">
                                            <label for="peminjaman_anggota_id"
                                                class="inline-block mb-2 text-base font-medium">Nama Anggota</label>
                                            <select id="peminjaman_anggota_id" name="peminjaman_anggota_id"
                                                class="form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                required>
                                                @foreach ($anggota as $value)
                                                    @if (!$value->is_delete)
                                                        <option value="{{ $value->anggota_id }}">{{ $value->anggota_nama }}
                                                        </option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="xl:col-span-6">
                                            <label for="peminjaman_buku_id"
                                                class="inline-block mb-2 text-base font-medium">Judul Buku</label>
                                            <select id="peminjaman_buku_id" name="peminjaman_buku_id"
                                                class="form-select border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                required>
                                                @foreach ($buku as $value)
                                                    @if (!$value->is_delete)
                                                        <option value="{{ $value->buku_id }}">{{ $value->buku_judul }}
                                                        </option>
                                                    @endif
                                                @endforeach
                                            </select>
                                        ...
                                        <div class="xl:col-span-6">
                                            <label for="peminjaman_tgl"
                                                class="inline-block mb-2 text-base font-medium">Tanggal Pinjam</label>
                                            <input type="date" id="peminjaman_tgl" name="peminjaman_tgl"
                                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                required>
                                        </div>
                                        <div class="xl:col-span-6">
                                            <label for="peminjaman_tgl_pengembalian"
                                                class="inline-block mb-2 text-base font-medium">Tanggal Kembali</label>
                                            <input type="date" id="peminjaman_tgl_pengembalian"
                                                name="peminjaman_tgl_pengembalian"
                                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                required>
                                        </div>
                                        <div class="xl:col-span-6">
                                            <label for="peminjaman_denda"
                                                class="inline-block mb-2 text-base font-medium">Denda</label>
                                            <input type="text" id="peminjaman_denda" name="peminjaman_denda"
                                                class="form-input border-slate-200 dark:border-zink-500 focus:outline-none focus:border-custom-500 disabled:bg-slate-100 dark:disabled:bg-zink-600 disabled:border-slate-300 dark:disabled:border-zink-500 dark:disabled:text-zink-200 disabled:text-slate-500 dark:text-zink-100 dark:bg-zink-700 dark:focus:border-custom-800 placeholder:text-slate-400 dark:placeholder:text-zink-200"
                                                readonly>
                                        </div>
                                        <div class="xl:col-span-6">
                                    ...
                                    <div class="flex justify-end mt-6 gap-x-4">
                                        ...
                                        <button type="submit" formaction="{{ route('admin.peminjaman.cetak_nota') }}"
